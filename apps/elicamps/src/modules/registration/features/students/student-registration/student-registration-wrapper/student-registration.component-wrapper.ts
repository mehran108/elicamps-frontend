...
export class StudentRegistrationWrapperComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public getStudentPayments = (studentId: any) => {
    this.groupService.getAllPaymentStudentByStudentId(studentId).subscribe((res: any) => {
      ...
      if (this.groupPaymentList.length > 0) {
        const paidValues: any = this.groupPaymentList.map((a: any) => a['amount']);
        const paid = paidValues.reduce((a: any, b: any) => a + b);
  ...
  public initializeStudentFormWithValues(student: any) {
    ...
    this.paymentInf.calculate();
    const campus = this.studentForm.controls['campus'].value;
    this.studentForm.controls['roomSearchCampus'].setValue(campus);
  }
  public populateNumberOfNights = (student: any) => {
  ...
  public getGroupList = () => {
    ...
    this.groupService.getAllElicampsGroups(params).subscribe((groupList: any) => {
  ...
  calcNumOfNights(value: any) {
    const student = this.studentForm.value;
    this.populateNumberOfNights(student);
  }
  applyDateChangeToField(dateEvent: any) {
    ...
    if (this.studentForm.valid) {
      ...
      if (this.isEdit === false) {
        ...
        this.listService.addStudentInfo(model).subscribe(res => {
          if (!isClose) {
            const studentId = res;
            const apiModel = this.clean(data);
            const updateMode = {
              ...apiModel,
              id: studentId
            }
