...
import { STUDENT_PAYMENT_COL_DEFS } from "apps/elicamps/src/common/elicamps-column-definitions";
...
export class PaymentInformationComponent implements OnInit {
  ...
  private gridApi: any;
  public columnDefs: any = STUDENT_PAYMENT_COL_DEFS;
...
this.gridOptions = {
  ...
  public getAddress() {
    ...
    if (this.studentForm.controls['homestayOrResi'].value == "1") {
      if (this.studentForm.controls['homestayID'].value) {
        const homeStay: any = this.homeStayList.find(
          (row: any) => row.homeId === this.studentForm.controls['homestayID'].value
        );
        if (homeStay) {
      ...
      const camp = this.studentForm.controls['roomSearchCampus'] || this.studentForm.controls['campus']
        const campus: any = this.campusList.find(
          (row: any) => row['id'] === camp.value
        );
        if (campus)
  ...
  public getEmailBody(selectedTemplate: any) {
  ...
  public sendEmail = () => {
    ...
    if (this.f.controls['studentEmail'].value && this.f.controls['emailBody'].value) {
      const selectedTemplate = this.templateList.find((row: any) => row.value === this.f.controls['emailBody'].value)
      const model = {
        studentId: this.studentForm.controls['id'].value,
        ...this.f.value,
        registrationFee: this.studentForm.controls['registrationFee'].value,
