...
export class BulkInvoiceComponent implements OnInit {
  ...
  public homeStayList: Array<any> = [];
  public campusList: Array<any> = [];
  public allStudentList: Array<any> = [];
  public studentEmail = '';
  public templateList = [
    {
      name: "Default",
  ...
  public getStudentList = () => {
    ...
    this.groupService
      ...
      .subscribe((studentList: any) => {
        this.studentList = ((studentList as any).data || []).map((el: any) => {
          return {
            ...el,
  ...
  public createEmailTasks(selectedTemplate: any) {
    ...
    this.selectedStudents.forEach(student => {
      const address = this.getAddress(student);
      const model = {
      ...
      const task = this.groupService.sendEmail(model);
      tasks.push(task);
    });
    return tasks;
  }
  public getEmailBody(selectedTemplate: any, student: any) {
