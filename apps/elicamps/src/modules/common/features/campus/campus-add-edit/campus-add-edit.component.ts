...
import { Campus, LookupTable } from 'apps/elicamps/src/models/Elicamps';
...
export class CampusAddEditComponent implements OnInit {
  ...
  public getSelectedAgent = (id: number) => {
    this.listService.getCampus(id).subscribe((res: any) => {
      if (res) {
  ...
  public initializeDropDowns = () => {
    this.listService.getAll(LookupEnum.CAMPS).subscribe((res: any) => {
      this.campsList = res;
    });
  }
  initializeForm() {
  ...
  onSubmit() {
    ...
    if (this.f.valid) {
      this.loading = true;
      if (this.isEdit) {
        this.listService.updateCampus(this.f.value).subscribe((res: any) => {
          this.loading = false;
          this.router.navigate(['/campus']);
        }, (error: any) => {
          this.loading = false;
        ...
        this.listService.addCampus(this.f.value).subscribe((res: any) => {
          this.loading = false;
          this.router.navigate(['/campus']);
        }, (error: any) => {
          this.loading = false;
